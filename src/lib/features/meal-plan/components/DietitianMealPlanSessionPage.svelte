<script lang="ts">
	import type {
		PageData,
		ActionData
	} from '../../../../routes/dietitian/meal-plan/[sessionId]/$types';
	import { DIAGS, MEAL_TYPES, DIET_TYPES, DAYS_W } from '$lib/meal-plan/constants';
	...
	import {
		...
		computePlanTotals,
		estimateMicros
	} from '$lib/meal-plan/macro-utils';
	import { validatePlan, type ValidationResult } from '$lib/meal-plan/validation';
	import {
	...
	let planStartDate = $state<string>(
		untrack(() => snapToSunday(data.session.startDate || todayStr()))
	);
	let viewWeekStart = $state<string>(
		untrack(() => snapToSunday(data.session.startDate || todayStr()))
	);
		...
		if (data.existingMealDays.length) {
			...
			for (const { day, meals: dayMeals } of data.existingMealDays) {
				const dateKey =
					...
					toLocalYmd(
						new Date(
							new Date(baseDate + 'T00:00:00').getTime() +
								(day.dayOfWeek ?? day.sortOrder) * 86400000
						)
					);
				restored[dateKey] = {};
				for (const meal of dayMeals) {
					...
					if ((meal as any).aiMealJson) {
						try {
							slot.aiMeal = JSON.parse((meal as any).aiMealJson);
						} catch {
							/* ignore invalid AI meal JSON */
						}
					}
	...
	const supplementLookup = $derived(() => {
		const map = new Map<
			number,
			{ totalKcal: number; protein: number; carbs: number; fat: number }
		>();
	...
	const planCalc = $derived(
		computePlanTotals(currentWeekPlan(), recipeLookup(), supplementLookup(), foodLookup())
	);
	...
	const sortedMealTypesForChips = $derived(() => {
		...
		return [...MEAL_TYPES].sort((a, b) => {
			...
			return (
				MEAL_TYPES.findIndex((x) => x.id === a.id) - MEAL_TYPES.findIndex((x) => x.id === b.id)
			);
		});
	});
	...
	const pickerItems = $derived(() => {
		...
		return data.recipes
			.filter(({ recipe }) => {
				if (!q) return true;
				return (
					recipe.name.toLowerCase().includes(q) || (recipe.nameAr ?? '').toLowerCase().includes(q)
				);
			})
			.map(({ recipe }) => {
				const n = parseNutrients(recipe.nutrients);
	...
	const detailRecipe = $derived(
		detailRecipeId
			? (data.recipes.find((r) => r.recipe.id === detailRecipeId)?.recipe ?? null)
			: null
	);
...
<div onclick={closeDropdowns}>
	<style>
		...
		.plan-btn.active {
			...
			background: #fff;
			color: #1f9e57;
			box-shadow: 0 2px 8px rgba(46, 165, 93, 0.16);
		}
		...
		.macro-header-row > span:last-child {
			flex-shrink: 0;
			max-width: 100%;
		}
		.macro-pill-row {
		...
		.macro-slider-row:last-child {
			margin-bottom: 0;
		}
		.macro-slider-row .slider {
			min-width: 0;
		}
		.meal-type-grid {
			display: grid;
		...
		.cal-target-stats > div {
			flex: 1 1 72px;
			min-width: 0;
		}
		.cal-target-stats .stat-start {
			text-align: start;
		}
		.cal-target-stats .stat-mid {
			text-align: center;
		}
		.cal-target-stats .stat-end {
			text-align: end;
		}
		.chart-macro-row {
			display: flex;
		...
		.meal-chip.in-view-period:not(.on) {
			box-shadow: inset 3px 0 0 0 #7dd3a0;
			background: #fafffe;
		}
		.meal-chip.in-view-period.on {
		...
		.chart-meal-ring {
			display: block;
			margin: 0 auto;
			filter: drop-shadow(0 2px 5px rgba(16, 24, 40, 0.07));
		}
		.micro-nums {
			font-size: 9.5px;
			color: #8b909a;
		...
		.ai-day-dot:nth-child(1) {
			animation-delay: 0s;
		}
		.ai-day-dot:nth-child(2) {
			animation-delay: 0.2s;
		}
		.ai-day-dot:nth-child(3) {
			animation-delay: 0.4s;
		}
		.ai-day-dot:nth-child(4) {
			animation-delay: 0.6s;
		}
		.ai-day-dot:nth-child(5) {
			animation-delay: 0.8s;
		}
		.ai-day-dot:nth-child(6) {
			animation-delay: 1s;
		}
		.ai-day-dot:nth-child(7) {
		...
		.ai-meal-total-seg.cal .ai-meal-total-val {
			color: #15803d;
		}
		.ai-meal-total-seg.prot .ai-meal-total-val {
			color: #dc2626;
		}
		.ai-meal-total-seg.carb .ai-meal-total-val {
			color: #2563eb;
		}
		.ai-meal-total-seg.fat .ai-meal-total-val {
			color: #d97706;
		}
		.ai-meal-total-lbl {
			display: block;
		...
		@media (max-width: 900px) {
			.builder-wrap {
				padding: 12px max(10px, env(safe-area-inset-left, 0px))
			...
			.topbar {
				...
				grid-template-areas:
					'back save'
					'identity identity';
				padding: 10px max(12px, env(safe-area-inset-left, 0px)) 12px
			...
			{#if planValidation.status !== 'pass' || saveStatus === 'saving' || saveStatus === 'saved'}
				<div class="topbar-extras">
					{#if planValidation.status !== 'pass'}
						<span
							style="display:inline-flex; align-items:center; gap:4px; font-size:11px; font-weight:700; padding:4px 12px; border-radius:20px;
						background:{planValidation.status === 'fail' ? '#fef2f2' : '#fffbeb'};
						color:{planValidation.status === 'fail' ? '#dc2626' : '#d97706'};"
						>
							<span
								style="width:7px; height:7px; border-radius:50%; background:{planValidation.status ===
								'fail'
									? '#dc2626'
									: '#d97706'};"
							></span>
	...
	{#if planValidation.status !== 'pass' && (planValidation.exclusionWarnings.length > 0 || planValidation.restrictionWarnings.length > 0 || !planValidation.macroValid)}
		...
		>
			<span
				style="font-size:12px; font-weight:800; color:{planValidation.status === 'fail'
					? '#dc2626'
					: '#d97706'}; flex-shrink:0;"
			>
				{planValidation.status === 'fail' ? '⛔ تحذير خطير' : '⚠️ يوجد تنبيهات'}
			</span>
			{#if planValidation.exclusionWarnings.length > 0}
				<span
					style="font-size:11.5px; color:#991b1b; background:#fee2e2; padding:3px 10px; border-radius:8px; font-weight:600;"
				>
			...
			{#each planValidation.restrictionWarnings as rw}
				<span
					style="font-size:11.5px; color:#92400e; background:#fef3c7; padding:3px 10px; border-radius:8px; font-weight:600;"
				>
					{rw.label}: {rw.isMinimum ? 'أقل من الحد الأدنى' : 'تجاوز الحد'} ({Math.round(
						rw.actual
					)}/{rw.limit}
					{rw.unit})
				</span>
	...
	<div class="builder-wrap">
		...
		<div class="form-panel">
			...
			<div class="form-card" style="padding-bottom:12px;">
				...
				>
					...
					<div
						style="display:flex; align-items:center; background:#f0f2f5; border-radius:10px; padding:3px; gap:2px;"
					>
						<button
							onclick={() => {
								planType = 'daily';
								viewWeekStart = todayStr();
							}}
						...
						<button
							onclick={() => {
								planType = 'weekly';
								viewWeekStart = snapToSunday(todayStr());
							}}
							style="padding:5px 14px; border-radius:8px; border:none; font-size:12px; font-weight:700; font-family:'Tajawal',sans-serif; cursor:pointer; transition:.15s;
					...
					{#if showDiagDrop}
						<div class="dropdown">
							{#each diagnosisOptions() as d}
								<button
									class="drop-item"
									class:selected={selectedDiags.includes(d.id)}
									onclick={() => {
										toggleDiag(d.id);
										showDiagDrop = false;
									}}
				...
				{#if selectedDiags.length > 0}
					<div class="diag-recs-card">
						<div style="display:flex; align-items:center; gap:7px; margin-bottom:10px;">
							<span
								style="font-size:11.5px; font-weight:700; color:#92400e; text-transform:uppercase; letter-spacing:.4px;"
								>التفاصيل والتوصيات</span
							>
						</div>
						{#each selectedDiags as diagId}
							{@const d = diagnosisLookup().get(diagId)}
							{#if d}
								<div
									style="border:1px solid #fde68a; border-radius:10px; background:#fff; margin-bottom:10px;"
								>
									<div
										style="display:flex; align-items:center; justify-content:space-between; gap:8px; padding:8px 10px;"
									>
										<button
											type="button"
										...
										{#if customDiags.some((x) => x.id === d.id)}
											<button
												...
												onclick={() => startEditDiagnosis(d.id)}>تعديل</button
											>
										{/if}
									</div>
										...
										<div style="padding:0 10px 10px;">
											{#if 'severity' in d}
												<div class="diag-info-grid" style="margin-bottom:8px;">
													<div>
														<div class="diag-info-label">الكود</div>
														<div class="diag-info-value">{d.code || '—'}</div>
													</div>
													<div>
														<div class="diag-info-label">الشدة</div>
														<div class="diag-info-value">
															{d.severity === 'mild'
																? 'خفيف'
																: d.severity === 'moderate'
																	? 'متوسط'
													...
													<div>
														<div class="diag-info-label">تاريخ التشخيص</div>
														<div class="diag-info-value">{d.diagnosedDate || '—'}</div>
													</div>
													<div>
														<div class="diag-info-label">الحالة</div>
														<div class="diag-info-value">
															{d.status === 'active'
																? 'نشط'
																: d.status === 'resolved'
											...
											{:else if d.recs?.length}
												{#each d.recs as rec}
													<div
														style="display:flex; gap:8px; font-size:12.5px; color:#44403c; margin-bottom:5px; line-height:1.55;"
													>
					...
					<div style="margin-top:12px; padding-top:12px; border-top:1px solid #fde68a;">
						<div
							style="font-size:10.5px; font-weight:700; color:#92400e; text-transform:uppercase; letter-spacing:.4px; margin-bottom:6px;"
						>
							ملاحظات الأخصائي
						</div>
						<textarea
							bind:value={diagNotes}
							placeholder="أضف ملاحظاتك الخاصة هنا…"
							style="width:100%; border:1.5px solid #fde68a; border-radius:8px; padding:8px 10px; font-size:12.5px; font-family:'Tajawal',sans-serif; resize:vertical; min-height:72px; outline:none; color:#44403c; background:#fffdf0; box-sizing:border-box;"
						></textarea>
					</div>
				{/if}
			</div>
			...
			<div class="form-card">
				...
				{#if targetCalories > 0}
					<div style="font-size:10.5px; color:#8b909a; margin-top:6px;">
						{targetCalories < 1200
							? '⚠️ أقل من الحد الأدنى الآمن (1200 سعرة)'
							: targetCalories > 5000
								? '⚠️ مرتفع جداً — تأكد من صحة القيمة'
								: `سيتم توزيع ${targetCalories} سعرة على الوجبات المحددة`}
					</div>
				{/if}
			</div>
			...
			<div class="form-card">
				...
				<div style="position:relative;" class="exc-drop-zone">
					...
					{#if showExcDrop}
						<div class="dropdown">
							<div
								style="padding:5px 12px 2px; font-size:10px; font-weight:700; color:#92400e; letter-spacing:.4px; background:#fffbf5; border-radius:8px 8px 0 0;"
							>
								مسببات الحساسية
							</div>
							{#each ALLERGEN_OPTS as allergen}
								<button
									class="drop-item {excludedFoods.includes(allergen.value) ? 'selected' : ''}"
									onclick={() => toggleExcluded(allergen.value)}
								>
									{allergen.value}
									{#if excludedFoods.includes(allergen.value)}<span class="drop-item-check">✓</span
										>{/if}
								</button>
							{/each}
						</div>
			...
			<div class="form-card">
				<div
					class="macro-header-row"
					style="font-size:12px; font-weight:600; color:#8b909a; margin-bottom:14px;"
				>
					<span>توزيع المغذيات الكبرى</span>
					<span
						style="font-size:11px; padding:3px 10px; border-radius:20px; font-weight:700; transition:all .2s;
				...
				<div class="macro-slider-row">
					<div
						style="width:8px; height:8px; border-radius:50%; background:#4e9af1; flex-shrink:0;"
					></div>
					...
					<input
						...
						max="100"
						value={macros.c}
						oninput={(e) => onSlider('c', +(e.target as HTMLInputElement).value)}
						style="background:linear-gradient(to left, #4e9af1 0%, #4e9af1 {macros.c}%, #e8eaed {macros.c}%);"
					/>
				</div>
				<div class="macro-slider-row">
					<div
					...
					<input
						...
						max="100"
						value={macros.p}
						oninput={(e) => onSlider('p', +(e.target as HTMLInputElement).value)}
						style="background:linear-gradient(to left, #ef4444 0%, #ef4444 {macros.p}%, #e8eaed {macros.p}%);"
					/>
				</div>
				<div class="macro-slider-row">
					<div
					...
					<input
						...
						max="100"
						value={macros.f}
						oninput={(e) => onSlider('f', +(e.target as HTMLInputElement).value)}
						style="background:linear-gradient(to left, #f59e0b 0%, #f59e0b {macros.f}%, #e8eaed {macros.f}%);"
					/>
				</div>
			</div>
			...
			<div class="form-card">
				...
				<div style="font-size:10.5px; color:#8b909a; margin-bottom:10px; line-height:1.4;">
					الشريط الجانبي: يوجد وجبة في {planType === 'weekly'
						? 'الأسبوع المعروض'
						: 'اليوم المعروض'}. الوجبات ذات المحتوى تُرتّب أولاً.
				</div>
				<div class="meal-type-grid">
					{#each sortedMealTypesForChips() as m}
						...
						>
							...
							<span
								style="width:22px; height:22px; border-radius:50%; border:2px solid {selectedMeals.includes(
									m.id
								)
									? '#3cb96b'
									: '#e8eaed'}; background:{selectedMeals.includes(m.id)
									? '#3cb96b'
									: '#fff'}; display:flex; align-items:center; justify-content:center; font-size:13px; color:{selectedMeals.includes(
									m.id
								)
									? '#fff'
									: 'transparent'};"
			...
			<div class="form-card plan-grid-card">
				<div
					style="display:flex; align-items:center; justify-content:space-between; margin-bottom:12px; gap:10px; flex-wrap:wrap;"
				>
					<div style="display:flex; align-items:center; gap:8px;">
						<div class="field-label" style="margin-bottom:0;">الخطة الغذائية</div>
						{#if todayDayIdx() >= 0}
							<span class="week-current-badge"
								>{planType === 'weekly' ? 'هذا الأسبوع' : 'اليوم'}</span
							>
						{/if}
					</div>
					{#if Object.values(plan).some((day) => Object.keys(day).length > 0)}
						<button
							style="font-size:11px; padding:5px 12px; border-radius:7px; border:1.5px solid #e8eaed; background:#f4f6f9; color:#8b909a; cursor:pointer; font-family:'Tajawal',sans-serif; font-weight:600; transition:.15s;"
							onclick={() => {
							...
							onmouseleave={(e) => {
								...
								(e.currentTarget as HTMLElement).style.color = '#8b909a';
							}}
						>
							مسح الكل
						</button>
				...
				{#if displayMealTypes().length === 0}
					<div
						style="text-align:center; padding:32px 16px; color:#b0b5c0; font-size:13px; font-weight:600; border:2px dashed #e8eaed; border-radius:12px;"
					>
						...
						<table class="plan-table" class:plan-table-weekly={planType === 'weekly'}>
							...
							<tbody>
								{#each displayMealTypes() as mealType}
									...
									<tr>
										<td
											class="plan-meal-label-cell"
											style="font-size:10px; font-weight:700; color:#8b909a; text-transform:uppercase; letter-spacing:.5px; white-space:nowrap; padding:0 6px; vertical-align:middle; width:72px;"
										>
											{mealInfo?.label}
										</td>
										{#each dayDates() as dateKey, i}
											...
											{#if info}
												<td
													...
													style="border:2px solid {todayDayIdx() === i
														? '#3cb96b'
														: '#d9dce3'}; padding:0; position:relative;"
												>
													<!-- svelte-ignore a11y_click_events_have_key_events -->
													<!-- svelte-ignore a11y_no_static_element_interactions -->
													<div
													...
													>
														<div
															class="meal-card-hero"
															style="background:{info.type === 'ai'
																? 'linear-gradient(135deg,#f3e8ff,#e8dfff)'
																: info.type === 'supplement'
																	? 'linear-gradient(135deg,#f0ebff,#e8dfff)'
																	: info.type === 'food'
																		? 'linear-gradient(135deg,#fff9ed,#fde68a44)'
																		: info.type === 'recipe' && info.imageUrl
																			? '#e8edf3'
																			: 'linear-gradient(135deg,#edf9f2,#c5edd8)'}; color:{info.type ===
															'ai'
																? '#7c3aed'
																: info.type === 'supplement'
																	? '#7c5cbf'
														...
														>
															{#if info.type === 'recipe' && info.imageUrl}
																...
																/>
																<span
																	style="position:relative; z-index:1; background:rgba(255,255,255,.88); padding:2px 8px; border-radius:6px; font-size:11px; font-weight:700; color:#16a34a; box-shadow:0 1px 4px rgba(0,0,0,.08);"
																	>وصفة</span
																>
															{:else}
																{info.type === 'ai'
													...
													<div class="cell-actions">
														<button
															...
															onclick={(e) => {
																e.stopPropagation();
																openPicker(mealType, dateKey);
															}}>تعديل</button
														>
														{#if planType === 'weekly'}
															<button
																class="cell-btn cell-btn-swap"
																title="تبديل"
																onclick={(e) => {
																	e.stopPropagation();
																	openSwap(mealType, dateKey);
																}}>⇄</button
															>
														{/if}
														<button
															class="cell-btn cell-btn-del"
															title="حذف"
															onclick={(e) => {
																e.stopPropagation();
																removeSlot(dateKey, mealType);
															}}>×</button
														>
													</div>
												</td>
											{:else}
												...
												<td
													class="meal-cell"
													class:today-col={todayDayIdx() === i}
													onclick={() => openPicker(mealType, dateKey)}
												>
			...
			<div style="display:flex; gap:8px; margin-top:16px;">
				<button
					style="flex:1; padding:14px; border-radius:10px; border:none; background:linear-gradient(135deg,#7c3aed,#a855f7); color:#fff; font-size:15px; font-weight:700; transition:.15s; letter-spacing:.2px; font-family:'Tajawal',sans-serif; {!selectedMeals.length ||
					(!targetCalories && planCalc.totals.calories <= 0)
		...
		<div class="right-panel">
			<div class="chart-card">
				<div style="font-size:13px; font-weight:700; color:#1a1d23; margin-bottom:14px;">
					ملخص الخطة الغذائية
				</div>

				<!-- Calorie Card -->
				<div
					style="background:linear-gradient(135deg, #3cb96b 0%, #2ea55d 100%); border-radius:10px; padding:14px 16px; color:#fff; margin-bottom:10px;"
				>
					<div
					...
					>
						...
						<span style="font-size:16px; font-weight:800;"
							>{Math.round(effectiveTargetCalories)} سعرة</span
						>
					</div>
					...
					{/if}
					<div
						style="height:7px; background:rgba(255,255,255,.3); border-radius:4px; overflow:hidden; margin-bottom:9px;"
					>
						<div
							style="height:100%; background:#fff; border-radius:4px; transition:width .4s ease; width:{pctDone}%;"
						></div>
					</div>
					<div class="cal-target-stats">
						<div class="stat-start" style="display:flex; flex-direction:column; gap:1px;">
							<span style="font-size:13px; font-weight:800;"
								>{Math.round(planCalc.totals.calories)}</span
							>
							<span>مستهلك</span>
						</div>
				...
				<div class="chart-macro-row">
					<div
						style="flex:1; border-radius:9px; padding:10px 6px 8px; text-align:center; border:1.5px solid #c5d9fb; background:#eef4ff;"
					>
						<div style="font-size:17px; font-weight:800; color:#4e9af1; margin-bottom:1px;">
							{mg.carbG}g
						</div>
						<div style="font-size:10px; color:#8b909a; font-weight:600; margin-bottom:5px;">
							كارب
						</div>
						<div style="font-size:9.5px; color:#8b909a; margin-bottom:4px;">
							{Math.round(planCalc.totals.carbs)} / {mg.carbG} g
						</div>
						<div style="height:4px; background:#e8eaed; border-radius:2px; overflow:hidden;">
							<div
								style="height:100%; border-radius:2px; background:#4e9af1; transition:width .4s; width:{mg.carbG >
								0
									? Math.min(100, Math.round((planCalc.totals.carbs / mg.carbG) * 100))
									: 0}%;"
							></div>
						</div>
					</div>
					<div
						style="flex:1; border-radius:9px; padding:10px 6px 8px; text-align:center; border:1.5px solid #fbbcbc; background:#fff0f0;"
					>
						<div style="font-size:17px; font-weight:800; color:#ef4444; margin-bottom:1px;">
							{mg.protG}g
						</div>
						<div style="font-size:10px; color:#8b909a; font-weight:600; margin-bottom:5px;">
							بروتين
						...
						<div style="font-size:9.5px; color:#8b909a; margin-bottom:4px;">
							{Math.round(planCalc.totals.protein)} / {mg.protG} g
						</div>
						<div style="height:4px; background:#e8eaed; border-radius:2px; overflow:hidden;">
							<div
								style="height:100%; border-radius:2px; background:#ef4444; transition:width .4s; width:{mg.protG >
								0
									? Math.min(100, Math.round((planCalc.totals.protein / mg.protG) * 100))
									: 0}%;"
							></div>
						</div>
					</div>
					<div
						style="flex:1; border-radius:9px; padding:10px 6px 8px; text-align:center; border:1.5px solid #fde68a; background:#fffbf0;"
					>
						<div style="font-size:17px; font-weight:800; color:#f59e0b; margin-bottom:1px;">
							{mg.fatG}g
						</div>
						<div style="font-size:10px; color:#8b909a; font-weight:600; margin-bottom:5px;">
							دهون
						...
						<div style="font-size:9.5px; color:#8b909a; margin-bottom:4px;">
							{Math.round(planCalc.totals.fat)} / {mg.fatG} g
						</div>
						<div style="height:4px; background:#e8eaed; border-radius:2px; overflow:hidden;">
							<div
								style="height:100%; border-radius:2px; background:#f59e0b; transition:width .4s; width:{mg.fatG >
								0
									? Math.min(100, Math.round((planCalc.totals.fat / mg.fatG) * 100))
									: 0}%;"
							></div>
						</div>
					</div>
				...
				<div style="margin-bottom:12px;">
					<div
						style="font-size:11px; font-weight:700; color:#8b909a; text-transform:uppercase; letter-spacing:.4px; margin-bottom:9px;"
					>
						الوجبات
					</div>
					{#if displayMealTypes().length === 0}
						<div style="text-align:center; color:#8b909a; font-size:11px; padding:6px 0;">
							لا توجد وجبات محددة بعد
						</div>
					{:else if chartMealTypes().length === 0}
						<div
							style="text-align:center; color:#8b909a; font-size:11px; padding:6px 0; line-height:1.45;"
						>
							لا توجد وجبات مسجّلة بسعرات في {planType === 'weekly' ? 'هذا الأسبوع' : 'هذا اليوم'} — أضف
							وجبات في الجدول أو انتقل إلى فترة فيها بيانات.
						</div>
					{:else}
						<div style="display:flex; flex-wrap:wrap; gap:8px;">
							{#each chartMealTypes() as mealType, mi}
								...
								{@const mealPct =
									planCalc.totals.calories && mt
										? Math.round((mt.calories / planCalc.totals.calories) * 100)
										: 0}
								{@const ringR = 17}
								...
								<div class="chart-meal-ring-wrap" style="text-align:center; min-width:58px;">
									...
									<div style="font-size:11px; font-weight:700; color:#1a1d23; margin-top:2px;">
										{mealPct}%
									</div>
									<div style="font-size:10px; color:#8b909a; margin-top:3px; font-weight:500;">
										{mealInfo?.label}
									</div>
								</div>
							{/each}
				...
				<div style="margin-bottom:12px;">
					...
					{#each microData.slice(0, 5) as micro}
						{@const barColor =
							micro.pct >= 80 ? '#3cb96b' : micro.pct >= 50 ? '#f59e0b' : '#ef4444'}
						<div class="micro-item">
							<div class="micro-name">{micro.label}</div>
							<div class="micro-track">
								<div
									style="height:100%; border-radius:3px; background:{barColor}; width:{micro.pct}%; transition:width .4s;"
								></div>
							</div>
					...
					{/if}
					<button
						onclick={() => (microExpanded = !microExpanded)}
						style="width:100%; border:1.5px solid #e8eaed; border-radius:8px; background:#fff; color:#8b909a; font-size:11px; font-weight:600; padding:6px; cursor:pointer; margin-top:4px; transition:.15s; font-family:'Tajawal',sans-serif;"
					>
				...
				<!-- Meal Detail Table -->
				<div
					style="font-size:11px; font-weight:700; color:#8b909a; text-transform:uppercase; letter-spacing:.4px; margin-bottom:8px;"
				>
					تفاصيل الوجبات
				</div>
				<div class="mdt-scroll">
					<table class="mdt">
						...
						<tbody>
							{#if displayMealTypes().length === 0}
								<tr
									><td colspan="5" style="text-align:center; color:#8b909a; padding:10px;"
										>لا توجد وجبات في الخطة بعد</td
									></tr
								>
							{:else if chartMealTypes().length === 0}
								<tr
									><td colspan="5" style="text-align:center; color:#8b909a; padding:10px;"
										>لا بيانات سعرات للوجبات في {planType === 'weekly'
											? 'هذا الأسبوع'
											: 'هذا اليوم'}</td
									></tr
								>
				...
				<!-- Tracking -->
				<a
					href="/dietitian/meal-plan/{data.session
						.id}/tracking?type={planType}&date={viewWeekStart}"
					...
					onmouseenter={(e) => {
						(e.currentTarget as HTMLAnchorElement).style.background = '#d6f5e6';
						(e.currentTarget as HTMLAnchorElement).style.borderColor = '#3cb96b';
					}}
					onmouseleave={(e) => {
						(e.currentTarget as HTMLAnchorElement).style.background = '#edf9f2';
						(e.currentTarget as HTMLAnchorElement).style.borderColor = '#c5edd8';
					}}
				>
	...
	<div class="overlay" onclick={(e) => e.target === e.currentTarget && (showRecipePicker = false)}>
		<div class="modal">
			...
			>
				...
				<button
					onclick={() => (showRecipePicker = false)}
					style="background:none; border:none; font-size:20px; cursor:pointer; color:#8b909a; padding:4px; line-height:1;"
					>×</button
				>
			</div>
			<p style="font-size:12px; color:#8b909a; margin:0 0 12px;">
				...
				{#if planType === 'weekly'}
					· {dayLabels()[dayDates().indexOf(pickerContext!.dateKey)]}{/if}
			</p>
			<div class="picker-tabs">
				<button
					class="picker-tab"
					class:active={pickerTab === 'recipe'}
					onclick={() => {
						pickerTab = 'recipe';
						pickerSearch = '';
					}}>وصفات ({data.recipes.length})</button
				>
				<button
					class="picker-tab"
					class:active={pickerTab === 'supplement'}
					onclick={() => {
						pickerTab = 'supplement';
						pickerSearch = '';
						pickerSupplCategory = 'all';
					}}>مكملات ({data.supplements.length})</button
				>
			</div>

			{#if pickerTab === 'supplement'}
				<div style="display:flex; flex-wrap:wrap; gap:4px; margin-bottom:10px;">
					{#each [{ key: 'all', label: 'الكل' }, { key: 'standard', label: 'قياسي' }, { key: 'fiber', label: 'ألياف' }, { key: 'clear_ons', label: 'شفاف' }, { key: 'high_calorie', label: 'عالي السعرات' }, { key: 'diabetic', label: 'سكري' }, { key: 'renal', label: 'كلوي' }, { key: 'hepatic', label: 'كبدي' }, { key: 'pulmonary', label: 'رئوي' }, { key: 'surgery', label: 'جراحي' }, { key: 'semielemental', label: 'شبه عنصري' }] as cat}
						<button
							style="padding:3px 9px; border-radius:12px; border:1.5px solid {pickerSupplCategory ===
							cat.key
								? '#3cb96b'
								: '#e8eaed'}; background:{pickerSupplCategory === cat.key
								? '#edf9f2'
								: '#fff'}; color:{pickerSupplCategory === cat.key
								? '#3cb96b'
								: '#8b909a'}; font-size:11px; font-weight:600; cursor:pointer; font-family:'Tajawal',sans-serif; transition:.13s; white-space:nowrap;"
			...
			<input
				...
				style="width:100%; border:1.5px solid #e8eaed; border-radius:9px; padding:9px 12px; font-size:13px; outline:none; margin-bottom:10px; font-family:'Tajawal',sans-serif; box-sizing:border-box;"
			/>
					...
					<button class="recipe-item" onclick={() => pickItem(item)}>
						{#if item.type === 'recipe' && 'imageUrl' in item && item.imageUrl}
							<img
								src={item.imageUrl}
								alt=""
								style="width:40px;height:40px;object-fit:cover;border-radius:8px;flex-shrink:0;border:1px solid #e8eaed;"
							/>
						{:else if item.type === 'supplement'}
							<div
								style="width:40px; height:40px; flex-shrink:0; border-radius:9px; background:linear-gradient(135deg,#f3e8ff,#ede9fe); display:flex; flex-direction:column; align-items:center; justify-content:center; gap:1px;"
							>
								<span style="font-size:11px; font-weight:800; color:#7c3aed; line-height:1;"
									>{(item as any).kcalPerMl ?? '—'}</span
								>
								<span
									style="font-size:7.5px; color:#9d6bef; font-weight:600; line-height:1; letter-spacing:.2px;"
									>kcal/mL</span
								>
							</div>
						{:else}
							<div
								style="width:40px; height:40px; flex-shrink:0; border-radius:9px; background:linear-gradient(135deg,#edf9f2,#c5edd8); display:flex; align-items:center; justify-content:center;"
							>
								<svg
									width="16"
									height="16"
						...
						<div style="flex:1; min-width:0;">
							...
							<div
								style="font-size:10px; color:#8b909a; margin-top:2px; display:flex; gap:5px; flex-wrap:wrap; align-items:center;"
							>
								...
								{#if (item as any).volumeMl}<span style="color:#64748b;"
										>{(item as any).volumeMl} mL</span
									>{/if}
								{#if (item as any).fiber}<span style="color:#059669; font-weight:600;"
										>ألياف: {(item as any).fiber}g</span
									>{/if}
								{#if (item as any).osmolarity}<span style="color:#94a3b8;"
										>{(item as any).osmolarity} mOsm/L</span
									>{/if}
							</div>
						</div>
						<div
							style="font-size:11px; color:#3cb96b; font-weight:700; white-space:nowrap; text-align:left;"
						>
							{Math.round(item.calories)} kcal
						</div>
					</button>
				{:else}
					<div style="text-align:center; padding:24px; color:#8b909a; font-size:13px;">
						لا توجد نتائج مطابقة
					</div>
				{/each}
			</div>
			<div style="margin-top:12px;">
				<button class="btn-close" onclick={() => (showRecipePicker = false)} style="width:100%;"
					>إلغاء</button
				>
			</div>
		</div>
	...
	<div class="overlay" onclick={(e) => e.target === e.currentTarget && (showMealDetail = false)}>
		<div class="modal" style="max-width:560px;">
			<h2 style="font-size:17px; font-weight:700; margin:0 0 4px;">
				{detailRecipe.nameAr ?? detailRecipe.name}
			</h2>
			<p style="font-size:12.5px; color:#8b909a; margin:0 0 6px;">
				{detailRecipe.portions ? detailRecipe.portions + ' حصة' : ''}{detailRecipe.source ===
				'internal'
					? ' - يدوية'
					: ''}
			</p>
			{#if detailRecipe.imageUrl}
				<div
					style="margin:10px 0 14px; border-radius:10px; overflow:hidden; border:1px solid #e8eaed;"
				>
					<img
			...
			{#if detailNutrients}
				<div style="display:flex; gap:8px; margin-bottom:18px; margin-top:10px;">
					<div
						style="flex:1; background:linear-gradient(135deg,#edf9f2,#d4f0e0); border-radius:9px; padding:10px 8px; text-align:center;"
					>
						<div style="font-size:18px; font-weight:800; color:#16a34a;">
							{Math.round(detailNutrients.calories ?? 0)}
						</div>
						<div style="font-size:10px; color:#6b7280; margin-top:2px;">سعرة</div>
					</div>
					<div
						style="flex:1; background:#f4f6f9; border-radius:9px; padding:10px 8px; text-align:center;"
					>
						<div style="font-size:17px; font-weight:700; color:#ef4444;">
							{Math.round(detailNutrients.protein ?? 0)}g
						</div>
					...
					<div
						style="flex:1; background:#f4f6f9; border-radius:9px; padding:10px 8px; text-align:center;"
					>
						<div style="font-size:17px; font-weight:700; color:#4e9af1;">
							{Math.round(detailNutrients.carbs ?? 0)}g
						</div>
					...
					<div
						style="flex:1; background:#f4f6f9; border-radius:9px; padding:10px 8px; text-align:center;"
					>
						<div style="font-size:17px; font-weight:700; color:#f59e0b;">
							{Math.round(detailNutrients.fat ?? 0)}g
						</div>
			...
			{#if detailRecipe.ingredientDetails?.length}
				<div
					style="font-size:12px; font-weight:700; color:#8b909a; text-transform:uppercase; letter-spacing:.4px; margin-bottom:6px;"
				>
					المكونات ({detailRecipe.ingredientDetails.length})
				</div>
				<div style="background:#f4f6f9; border-radius:8px; padding:8px 12px; margin-bottom:16px;">
					{#each detailRecipe.ingredientDetails as ing}
						<div
							style="display:flex; justify-content:space-between; padding:5px 0; border-bottom:1px solid #e8eaed; font-size:12.5px;"
						>
			...
			{#if detailRecipe.steps}
				...
				<div
					style="font-size:12.5px; color:#4b5563; line-height:1.65; background:#f4f6f9; border-radius:8px; padding:10px 12px; margin-bottom:16px; white-space:pre-wrap;"
				>
					{detailRecipe.steps}
				</div>
			{/if}
			<button class="btn-close" onclick={() => (showMealDetail = false)} style="width:100%;"
				>إغلاق</button
			>
		</div>
	</div>
	...
	<div class="overlay" onclick={(e) => e.target === e.currentTarget && (showSwapModal = false)}>
		<div class="modal">
			...
			<p style="font-size:12.5px; color:#8b909a; margin:0 0 16px;">
				{MEAL_TYPES.find((m) => m.id === sc.mealType)?.label} - {dayLabels()[
					dayDates().indexOf(sc.dateKey)
				]}
			</p>
			<div style="display:grid; grid-template-columns:1fr 1fr; gap:8px; margin-top:12px;">
				{#each dayDates() as dateKey, i}
					{#if dateKey !== sc.dateKey}
						...
						{@const info = slot ? getSlotInfo(slot) : null}
						<button
							style="border:1.5px solid #e8eaed; border-radius:10px; padding:11px 10px; cursor:pointer; text-align:center; transition:.13s; font-size:12px; background:#fff; font-family:'Tajawal',sans-serif;"
							onclick={() => doSwap(dateKey)}
							onmouseenter={(e) => {
								(e.currentTarget as HTMLElement).style.borderColor = '#3cb96b';
								(e.currentTarget as HTMLElement).style.background = '#edf9f2';
							}}
							onmouseleave={(e) => {
								(e.currentTarget as HTMLElement).style.borderColor = '#e8eaed';
			...
			<div style="margin-top:12px;">
				<button class="btn-close" onclick={() => (showSwapModal = false)} style="width:100%;"
					>إلغاء</button
				>
			</div>
		</div>
	...
	>
		<div class="modal" style="max-width:520px;">
			...
			<p style="font-size:12.5px; color:#8b909a; margin:0 0 12px; line-height:1.6;">
				سيتم استخدام بيانات المريض تلقائيا (التشخيص، الأطعمة المستثناة، السعرات، المغذيات الكبرى)
				لإنشاء خطة مخصصة.
			</p>

			<!-- Mode badge — locked to current planType, not user-selectable here -->
			<div
				style="display:inline-flex; align-items:center; gap:6px; padding:6px 14px; border-radius:20px; margin-bottom:14px;
				background:{planType === 'weekly' ? '#f3e8ff' : '#edf9f2'};
				border:1.5px solid {planType === 'weekly' ? '#c4b5fd' : '#cbeed9'};
				color:{planType === 'weekly'
					? '#7c3aed'
					: '#1f9e57'}; font-size:12px; font-weight:700; font-family:'Tajawal',sans-serif;"
			>
			...
			<div style="background:#f4f6f9; border-radius:10px; padding:12px; margin-bottom:14px;">
				<div
					style="font-size:11px; font-weight:700; color:#8b909a; margin-bottom:8px; text-transform:uppercase; letter-spacing:.4px;"
				>
					ملخص الإعدادات المستخدمة
				</div>
				<div
					style="display:grid; grid-template-columns:1fr 1fr; gap:6px; font-size:12px; color:#4b5563;"
				>
					...
					{#if selectedMeals.filter((m) => m !== 'supplement').length}
						<div style="grid-column:1/-1;">
							...
							{#each selectedMeals.filter((m) => m !== 'supplement') as mId}
								<span
									style="display:inline-block; background:#e9d5ff; color:#7c3aed; border-radius:6px; padding:1px 8px; font-size:11px; font-weight:700; margin:1px 2px;"
								>
			...
			<div style="display:flex; flex-wrap:wrap; gap:8px; margin-bottom:14px;">
				{#each DIET_TYPES as dt}
					<button
						class="diet-tag"
						class:on={selectedDietTypes.includes(dt)}
						onclick={() => toggleDietType(dt)}>{dt}</button
					>
				{/each}
			</div>
			...
			<textarea
				...
				style="width:100%; border:1.5px solid #e8eaed; border-radius:9px; padding:10px 13px; font-size:13px; font-family:'Tajawal',sans-serif; resize:vertical; min-height:72px; outline:none; color:#1a1d23; margin-bottom:4px; box-sizing:border-box;"
			></textarea>
			...
			<div style="display:flex; gap:8px; margin-top:8px;">
				<button class="btn-close" onclick={() => (showAiModal = false)} disabled={aiGenerating}
					>إلغاء</button
				>
				<button
...
{#if showAiConfirm && aiGeneratedPlan}
	...
	<div class="overlay" onclick={(e) => e.target === e.currentTarget && (showAiConfirm = false)}>
		<div class="modal" style="max-width:560px;">
			...
			<p style="font-size:12.5px; color:#8b909a; margin:0 0 16px;">
				{aiGeneratedPlan.days?.length ?? 0} يوم — {aiGeneratedPlan.days?.reduce(
					(s: number, d: any) => s + (d.meals?.length ?? 0),
					0
				) ?? 0} وجبة
			</p>
			{#if aiPlanPartial && aiPlanFailedDates.length > 0}
				...
				>
					...
					<div style="margin-top:6px; font-weight:700; direction:ltr; text-align:right;">
						{aiPlanFailedDates.join('، ')}
					</div>
				</div>
			{/if}
			...
			<div style="max-height:300px; overflow-y:auto; margin-bottom:16px;">
				{#each aiGeneratedPlan.days ?? [] as day}
					<div
						style="border:1px solid #e8eaed; border-radius:10px; padding:10px 12px; margin-bottom:8px;"
					>
						<div style="font-size:12px; font-weight:800; color:#7c3aed; margin-bottom:6px;">
							{day.date}
						</div>
						{#each day.meals ?? [] as meal}
							<div
								style="display:flex; justify-content:space-between; padding:4px 0; border-bottom:1px solid #f0f0f0; font-size:12px;"
							>
								<span style="font-weight:600; color:#1f2937;">{meal.name_ar}</span>
								<span style="color:#3cb96b; font-weight:700;"
									>{Math.round(meal.total?.calories ?? 0)} سعرة</span
								>
							</div>
						{/each}
					</div>
			...
			<div
				style="background:#fffbeb; border:1.5px solid #fde68a; border-radius:10px; padding:10px 12px; font-size:12px; color:#92400e; margin-bottom:14px;"
			>
				سيتم استبدال الوجبات الحالية في الأيام المحددة. هذه الوجبات خاصة بهذا العميل ولن تُضاف إلى
				مكتبة الوصفات.
			</div>
			...
			<div style="display:flex; gap:8px;">
				...
				>
				<button class="btn-generate" style="width:auto; flex:2; padding:11px;" onclick={applyAiPlan}
					>تطبيق الخطة ←</button
				>
			</div>
		</div>
...
{#if showCreateDiagModal}
	...
	<div
		class="overlay"
		onclick={(e) => e.target === e.currentTarget && (showCreateDiagModal = false)}
	>
		<div class="modal" style="max-width:560px;">
			<h2 style="font-size:17px; font-weight:700; margin:0 0 4px;">إضافة تشخيص جديد</h2>
			<p style="font-size:12.5px; color:#8b909a; margin:0 0 16px;">
				سيتم حفظه في السجل الطبي للمريض ويمكن اختياره مباشرةً في الخطة.
			</p>
			...
			<div style="display:grid; grid-template-columns:1fr 1fr; gap:10px; margin-bottom:10px;">
				<div style="grid-column:1 / -1;">
					<button
						type="button"
						class="diag-section-btn"
						onclick={() => (createDiagBasicOpen = !createDiagBasicOpen)}
					>
					...
					{#if createDiagBasicOpen}
						<div style="display:grid; grid-template-columns:1fr 1fr; gap:10px;">
							<div style="grid-column:1 / -1;">
								<div style="font-size:11px; font-weight:700; color:#8b909a; margin-bottom:5px;">
									اسم التشخيص *
								</div>
								<input
									bind:value={newDiagName}
									placeholder="مثال: السكري النوع الثاني"
									style="width:100%; border:1.5px solid #e8eaed; border-radius:8px; padding:9px 10px; font-size:13px; font-family:'Tajawal',sans-serif; box-sizing:border-box;"
								/>
							</div>
							<div>
								<div style="font-size:11px; font-weight:700; color:#8b909a; margin-bottom:5px;">
							...
							<div>
								...
								<select
									bind:value={newDiagSeverity}
									style="width:100%; border:1.5px solid #e8eaed; border-radius:8px; padding:9px 10px; font-size:13px; font-family:'Tajawal',sans-serif; background:#fff; box-sizing:border-box;"
								>
				...
				<div style="grid-column:1 / -1;">
					<button
						type="button"
						class="diag-section-btn"
						onclick={() => (createDiagClinicalOpen = !createDiagClinicalOpen)}
					>
				...
				<div style="grid-column:1 / -1;">
					<button
						type="button"
						class="diag-section-btn"
						onclick={() => (createDiagNotesOpen = !createDiagNotesOpen)}
					>
						<span>الملاحظات</span>
					...
					{#if createDiagNotesOpen}
						<div>
							...
							<textarea
								bind:value={newDiagNotes}
								placeholder="أضف ملاحظات التشخيص..."
								style="width:100%; min-height:82px; border:1.5px solid #e8eaed; border-radius:8px; padding:9px 10px; font-size:13px; font-family:'Tajawal',sans-serif; resize:vertical; box-sizing:border-box;"
							></textarea>
						</div>
					{/if}
				</div>
			...
			<div style="display:flex; gap:8px;">
				...
				<button
					class="btn-generate"
					style="width:auto; flex:2; padding:11px;"
					disabled={isCreatingDiag}
