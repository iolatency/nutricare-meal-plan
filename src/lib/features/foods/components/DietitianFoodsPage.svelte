<script lang="ts">
	...
	function selectFood(
		...
		try {
			if (food.externalParserMeasuresJson) {
				measures = JSON.parse(food.externalParserMeasuresJson).map((m: any) => ({
					label: m.label,
					weight: m.weight
				}));
			}
	...
	function parsedMicros(fn: string | null): Record<string, number> {
		if (!fn) return {};
		try {
			return JSON.parse(fn) as Record<string, number>;
		} catch {
			return {};
		}
	}
	...
	function isRedundantSecondaryName(
		primary: string,
	...
	async function searchExternal() {
		...
		searchTimer = setTimeout(async () => {
			...
			try {
				// 1. Search locally imported foods first
				const localData = await searchLocalFoods(externalQuery);
				if (localData.length > 0) {
					externalLocalResults = localData.map((f) => ({ ...f, categoryNameAr: null }));
					externalLocalPage = 1;
					externalResults = [];
					externalResultSource = 'local';
					return; // Local results found — no need to call the API
				}
				// 2. No local results — fall back to Edamam API
	...
	function openEditFood(food: SelectedFoodInfo) {
		...
		eMicros = nextMicros;
		eCautions = Array.isArray((micros as any)._cautions)
			? ((micros as any)._cautions as string[])
			: [];
		eDietLabels = Array.isArray((micros as any)._diet_labels)
			? ((micros as any)._diet_labels as string[])
			: [];
		eHealthLabels = Array.isArray((micros as any)._health_labels)
			? ((micros as any)._health_labels as string[])
			: [];
	...
	const nutrientFootnote = $derived(
		fUnit === 'g'
			? 'القيم الغذائية أدناه لكل 100 غرام.'
			: fUnit === 'ml'
				? 'القيم الغذائية أدناه لكل 100 مل.'
...
<div class="page" dir="rtl">
	...
	<div class="tabs-wrap">
		...
		>
		<button
			class="tab-btn {activeTab === 'external' ? 'active' : ''}"
			onclick={() => (tabOverride = 'external')}>قاعدة بيانات خارجية</button
		>
	</div>
	...
	{#if activeTab === 'internal'}
		...
		<form method="GET" class="food-search-form">
			<div class="search-field-wrap">
				<svg
					style="position:absolute; right:13px; top:50%; transform:translateY(-50%); width:15px; height:15px; color:#8b909a; pointer-events:none;"
					fill="none"
					stroke="currentColor"
					viewBox="0 0 24 24"
				>
		...
		<div class="food-list">
					...
					<div style="display:flex; align-items:center; gap:10px; flex:1; min-width:0;">
						...
						{:else}
							<div class="food-thumb-ph" aria-hidden="true">
								<svg width="14" height="14" fill="none" stroke="#c0c7d0" viewBox="0 0 24 24"
									><path
										stroke-linecap="round"
										stroke-linejoin="round"
										stroke-width="1.5"
						...
						>
							...
							<p class="food-meta">
								<span class="badge {food.source === 'edamam' ? 'badge-external' : 'badge-internal'}"
									>{food.source === 'edamam' ? 'قاعدة خارجية' : 'أطعمتي'}</span
								>
								{#if category}
									<span>·</span>
			...
			{:else}
				<div style="text-align:center; padding:48px; color:#8b909a; font-size:13px;">
					{data.q
						? `لا توجد نتائج لـ "${data.q}"`
						: 'لا توجد أطعمة — أضف أول طعام أو استورد من البحث الخارجي'}
				</div>
			{/each}
		</div>
		...
		<div class="pagination-wrap">
			...
			<div style="display:flex; gap:8px;">
				<a
					...
					href={`/dietitian/foods?q=${encodeURIComponent(data.q)}&page=${Math.max(1, data.page - 1)}`}
					>السابق</a
				>
				<a
					class="pagination-btn {data.page >= data.totalPages ? 'disabled' : ''}"
					href={`/dietitian/foods?q=${encodeURIComponent(data.q)}&page=${Math.min(data.totalPages, data.page + 1)}`}
					>التالي</a
				>
			</div>
		</div>
	{:else}
		...
		<div style="display:flex; gap:10px; margin-bottom:16px; flex-wrap:wrap; align-items:center;">
			<div class="search-field-wrap" style="flex:1; min-width:0; max-width:560px;">
				...
				<input
					...
					oninput={searchExternal}
					placeholder="ابحث عن أي طعام بالإنجليزي للاستيراد..."
					class="input"
					style="padding-right:42px;"
				/>
			</div>
			{#if externalQuery}
				<button
					...
					style="background:none; border:none; cursor:pointer; color:#8b909a; font-size:13px; font-family:'Tajawal',sans-serif; padding:0;"
					>مسح</button
				>
			{/if}
		</div>
		...
		{#if externalLoading}
			<div
				style="display:flex; align-items:center; justify-content:center; gap:10px; padding:40px; color:#8b909a; font-size:13px;"
			>
				<div class="spinner"></div>
				{externalResultSource === 'local'
					? 'جاري البحث في قاعدتك...'
					: 'لا نتائج محلية — جاري البحث في Edamam...'}
			</div>
		{:else if externalQuery.length >= 2}
			{#if externalResultSource === 'local' && externalLocalResults.length > 0}
				...
				<p style="font-size:12px; color:#8b909a; margin:0 0 10px;">
					...
					{#if externalLocalTotalPages > 1}
						— عرض {(externalLocalPage - 1) * EXTERNAL_LOCAL_PAGE_SIZE + 1}–{Math.min(
							externalLocalPage * EXTERNAL_LOCAL_PAGE_SIZE,
							externalLocalResults.length
						)} من {externalLocalResults.length}
					{/if}
				</p>
				<div class="food-list">
					{#each externalLocalResultsPaged as food}
						...
						>
							...
							<div class="macro-chips">
								<div class="macro-chip">
									<span class="macro-val">{food.calories}</span><span class="macro-lbl">kcal</span>
								</div>
								<div class="macro-chip">
									<span class="macro-val" style="color:#f59e0b;">{food.fat}g</span><span
										class="macro-lbl">دهون</span
									>
								</div>
								<div class="macro-chip">
									<span class="macro-val" style="color:#3cb96b;">{food.carbs}g</span><span
										class="macro-lbl">كارب</span
									>
								</div>
								<div class="macro-chip">
									<span class="macro-val" style="color:#7c5cbf;">{food.protein}g</span><span
										class="macro-lbl">بروتين</span
									>
								</div>
							</div>
						</div>
				...
				{#if externalLocalTotalPages > 1}
					<div class="pagination-wrap" style="margin-top:16px;">
						...
						<div style="display:flex; gap:8px;">
							...
							<button
								type="button"
								class="pagination-btn {externalLocalPage >= externalLocalTotalPages
									? 'disabled'
									: ''}"
			...
			{:else if externalResultSource === 'api' && externalResults.length > 0}
				...
				<div class="ext-list">
					{#each externalResultsPaged as food}
						...
						>
							...
							{:else}
								<div class="food-thumb-ph">
									<svg width="16" height="16" fill="none" stroke="#c0c7d0" viewBox="0 0 24 24"
										><path
											stroke-linecap="round"
											stroke-linejoin="round"
											stroke-width="1.5"
							...
							<div style="flex:1; min-width:0;">
								<p
									style="font-weight:600; font-size:13.5px; color:#1a1d23; margin:0; white-space:nowrap; overflow:hidden; text-overflow:ellipsis;"
								>
									{food.label}
								</p>
							...
							>
								...
								{:else if isImporting}
									<button
										...
										class="btn-import"
										disabled
										aria-label="جاري الاستيراد"
										title="جاري الاستيراد"><div class="spinner"></div></button
									>
								{:else}
					...
					<div class="pagination-wrap" style="margin-top:16px;">
						...
						<div style="display:flex; gap:8px;">
							<button
								...
								onclick={() => (externalApiPage = Math.max(1, externalApiPage - 1))}>السابق</button
							>
							<button
								type="button"
				...
				{/if}
			{:else}
				<div style="text-align:center; padding:40px; color:#8b909a; font-size:13px;">
					لا توجد نتائج لـ "{externalQuery}"
				</div>
			{/if}
		{:else}
			...
			{#if data.externalTotalCount > 0}
				<div class="food-list">
					{#each data.externalFoods as row}
						...
						{@const rowImg = foodRowImageUrl(
							isImported
								...
								: {
										...
										externalParserFoodJson: catalog.externalParserFoodJson
									}
						)}
						{@const displayName = isImported
							? (food!.nameAr ?? food!.name)
							: (catalog.nameAr ?? catalog.name)}
						...
						>
							...
							<div class="macro-chips">
								<div class="macro-chip">
									<span class="macro-val">{kcal}</span><span class="macro-lbl">kcal</span>
								</div>
								<div class="macro-chip">
									<span class="macro-val" style="color:#f59e0b;">{fatv}g</span><span
										class="macro-lbl">دهون</span
									>
								</div>
								<div class="macro-chip">
									<span class="macro-val" style="color:#3cb96b;">{carbsv}g</span><span
										class="macro-lbl">كارب</span
									>
								</div>
								<div class="macro-chip">
									<span class="macro-val" style="color:#7c5cbf;">{protv}g</span><span
										class="macro-lbl">بروتين</span
									>
								</div>
							</div>
				...
				{#if data.externalTotalPages > 1}
					<div class="pagination-wrap">
						<div style="font-size:12px; color:#8b909a;">
							صفحة {data.externalPage} من {data.externalTotalPages}
						</div>
						<div style="display:flex; gap:8px;">
							<a
								class="pagination-btn {data.externalPage <= 1 ? 'disabled' : ''}"
								href={`/dietitian/foods?extPage=${Math.max(1, data.externalPage - 1)}`}>السابق</a
							>
							<a
								class="pagination-btn {data.externalPage >= data.externalTotalPages
									? 'disabled'
									: ''}"
			...
			{:else}
				<div style="text-align:center; padding:56px 20px; color:#8b909a; font-size:13px;">
					...
					<p style="margin:0 0 4px; font-weight:500; color:#6b7280; font-size:14px;">
						لا توجد أطعمة في الكتالوج الخارجي بعد
					</p>
					<p style="margin:0; font-size:12.5px;">
						ابحث بالإنجليزي أعلاه — تُحفظ نتائج Edamam في القائمة ويمكنك استيرادها لاحقًا إلى أطعمتي
					</p>
				</div>
			{/if}
		{/if}
...
{#if selectedExternalFood}
	...
	<div
		...
		onclick={(e) =>
			...
			((selectedExternalFood = null), (externalDetailLoading = false))}
	>
		<div class="modal" style="max-width:520px;">
				...
				<div class="modal-header-food">
					<div class="modal-header-food-main">
						...
						<div class="modal-header-food-text">
							<h2
								style="font-size:16px; font-weight:700; color:#1a1d23; margin:0 0 2px; overflow:hidden; text-overflow:ellipsis; white-space:nowrap;"
							>
								{efHeading}
							</h2>
							...
							{/if}
							<div
								style="display:flex; align-items:center; gap:6px; margin-top:4px; flex-wrap:wrap;"
							>
					...
					<button
						...
						aria-label="إغلاق"
						title="إغلاق"
						onclick={() => ((selectedExternalFood = null), (externalDetailLoading = false))}
					>
			...
			<div class="modal-body">
				...
				{#if externalDetailLoading}
					<div
						style="display:flex; align-items:center; gap:10px; font-size:12.5px; color:#6b7280; padding:8px 12px; background:#f0f9ff; border:1px solid #bae6fd; border-radius:8px;"
					>
				...
				<div class="detail-macro-grid">
					<div class="detail-macro-tile" style="background:#fff8f0; border:1px solid #fed7aa;">
						<p style="font-size:18px; font-weight:700; color:#ea580c; margin:0 0 2px;">
							{ef.calories}<span style="font-size:12px; font-weight:600;"> kcal</span>
						</p>
					...
					<div class="detail-macro-tile" style="background:#faf5ff; border:1px solid #e9d5ff;">
						<p style="font-size:18px; font-weight:700; color:#7c3aed; margin:0 0 2px;">
							{ef.protein}g
						</p>
					...
					<div class="detail-macro-tile" style="background:#f0fdf4; border:1px solid #bbf7d0;">
						<p style="font-size:18px; font-weight:700; color:#16a34a; margin:0 0 2px;">
							{ef.carbs}g
						</p>
				...
				{#if ef.fiber > 0}
					<div
						style="display:flex; align-items:center; justify-content:space-between; padding:8px 12px; background:#f9fafb; border-radius:8px; font-size:13px;"
					>
				...
				{#if Object.keys(efMicros).filter((k) => !k.startsWith('_')).length > 0}
					<div>
						<p
							style="font-size:12px; font-weight:600; color:#6b7280; text-transform:uppercase; letter-spacing:.5px; margin:0 0 8px;"
						>
							المغذيات الدقيقة
						</p>
							...
							{#each MICRO_FIELDS as field}
								{#if !(field.key === 'FIBTG' && ef.fiber > 0)}
									...
									{#if val !== undefined && val !== 0}
										<div
											style="display:flex; align-items:center; justify-content:space-between; gap:8px; padding:6px 10px; background:#f9fafb; border-radius:7px; font-size:12.5px;"
										>
				...
				{#if extTags.cautions.length || extTags.dietLabels.length || extTags.healthLabels.length}
					<div class="section-card" style="padding:0; overflow:visible;">
						<div
							style="padding:10px 14px; border-bottom:1px solid #e8eaed; background:#f9fafb; border-radius:10px 10px 0 0;"
						>
							<p style="font-size:13px; font-weight:600; color:#1a1d23; margin:0 0 2px;">
								التصنيفات والعلامات
							</p>
							<p style="font-size:11px; color:#8b909a; margin:0;">
								من قاعدة البيانات الخارجية لهذا الطعام.
							</p>
						</div>
						<div style="padding:14px; display:flex; flex-direction:column; gap:14px;">
							{#if extTags.cautions.length}
								<div>
									<p style="font-size:11.5px; font-weight:600; color:#92400e; margin:0 0 8px;">
										تحذيرات (مسببات حساسية)
									</p>
									<div style="display:flex; flex-wrap:wrap; gap:5px;">
										{#each extTags.cautions as c}
											<span
												style="padding:2px 9px; background:#fef3c7; color:#92400e; border-radius:20px; font-size:11.5px; border:1px solid #fde68a;"
												>{tagLabelAr(c)}</span
											>
										{/each}
									</div>
							...
							{#if extTags.dietLabels.length}
								<div>
									<p style="font-size:11.5px; font-weight:600; color:#065f46; margin:0 0 8px;">
										التصنيف الغذائي
									</p>
									<div style="display:flex; flex-wrap:wrap; gap:5px;">
										{#each extTags.dietLabels as d}
											<span
												style="padding:2px 9px; background:#d1fae5; color:#065f46; border-radius:20px; font-size:11.5px; border:1px solid #6ee7b7;"
												>{tagLabelAr(d)}</span
											>
										{/each}
									</div>
							...
							{#if extTags.healthLabels.length}
								<div>
									<p style="font-size:11.5px; font-weight:600; color:#0c4a6e; margin:0 0 8px;">
										خالٍ من / مناسب لـ
									</p>
									<div style="display:flex; flex-wrap:wrap; gap:5px;">
										{#each extTags.healthLabels as h}
											<span
												style="padding:2px 9px; background:#e0f2fe; color:#0c4a6e; border-radius:20px; font-size:11.5px; border:1px solid #7dd3fc;"
												>{tagLabelAr(h)}</span
											>
										{/each}
									</div>
			...
			<div class="modal-footer">
				<p style="margin:0; font-size:12px; color:#8b909a; max-width:240px; line-height:1.45;">
					استورد هذا الطعام لإضافته إلى قاعدة بياناتك.
				</p>
				<div style="display:flex; gap:8px;">
					<button
						onclick={() => ((selectedExternalFood = null), (externalDetailLoading = false))}
						style="padding:8px 18px; border:1px solid #e8eaed; border-radius:8px; background:#fff; font-size:13.5px; color:#4b5563; cursor:pointer; font-family:'Tajawal',sans-serif;"
						>إغلاق</button
					>
					{#if isImporting}
						<button class="btn-primary" disabled style="opacity:.6; cursor:not-allowed; gap:8px;"
							><div class="spinner"></div>
							جاري...</button
						>
					{:else}
...
{#if selectedFood}
	...
	<div
		class="modal-overlay"
		role="presentation"
		onclick={(e) => e.target === e.currentTarget && (selectedFood = null)}
	>
		<div class="modal" style="max-width:520px;">
			<!-- Header -->
			<div class="modal-header">
				<div class="modal-header-food">
					<div class="modal-header-food-main">
						...
						<div class="modal-header-food-text">
							...
							>
								<span
									class="badge {selectedFood.source === 'edamam'
										? 'badge-external'
										: 'badge-internal'}"
									>{selectedFood.source === 'edamam' ? 'قاعدة خارجية' : 'أطعمتي'}</span
								>
								{#if selectedFood.categoryNameAr}<span style="font-size:11.5px; color:#8b909a;"
										>· {selectedFood.categoryNameAr}</span
									>{/if}
							</div>
						</div>
					</div>
			...
			<div class="modal-body">
				...
				<p class="detail-portion-strip">
					القيم الغذائية لكل {selectedFood.portionSize}
					{selectedFood.unit === 'g'
						? 'غرام'
				...
				<div class="detail-macro-grid">
					...
					<div class="detail-macro-tile" style="background:#fffbeb; border:1px solid #fde68a;">
						<p style="font-size:18px; font-weight:700; color:#d97706; margin:0 0 2px;">
							{selectedFood.fat}g
						</p>
						<p style="font-size:11px; color:#92400e; margin:0;">دهون</p>
					</div>
						...
						<div style="display:grid; grid-template-columns:1fr 1fr; gap:4px;">
							{#each MICRO_FIELDS as field}
								{#if !(field.key === 'FIBTG' && selectedFood.fiber > 0)}
									...
									{#if val !== undefined && val !== 0}
										<div
											style="display:flex; align-items:center; justify-content:space-between; padding:6px 10px; background:#f9fafb; border-radius:7px; font-size:12.5px;"
										>
											<span style="color:#6b7280;">{field.label}</span>
											<span style="font-weight:600; color:#1a1d23;"
												>{typeof val === 'number'
													? Number.isInteger(val)
														? val
														: val.toFixed(1)
		...
		<div class="modal">
			...
			<div class="modal-header">
				<div style="display:flex; align-items:flex-start; justify-content:space-between;">
					<div>
						<h2 style="font-size:17px; font-weight:700; color:#1a1d23; margin:0 0 3px;">
							إضافة طعام جديد
						</h2>
						<p style="font-size:12px; color:#8b909a; margin:0;">
							أضف عنصر غذائي مخصص لقاعدة البيانات
						</p>
					</div>
					<button
						type="button"
						...
						style="background:none; border:none; cursor:pointer; color:#8b909a; padding:2px; margin-top:2px;"
					>
						<svg width="18" height="18" fill="none" stroke="currentColor" viewBox="0 0 24 24"
							><path
								stroke-linecap="round"
			...
			>
				<div class="modal-body">
					...
					<div style="display:flex; align-items:center; gap:16px;">
						...
						/>
						<div>
							<p style="font-size:12px; font-weight:600; color:#059669; margin:0 0 2px;">
								صورة الطعام
							</p>
							<p style="font-size:11px; color:#9ca3af; margin:0 0 6px;">
								اختياري · PNG أو JPG · حتى 5 ميغابايت
							</p>
							<button
								type="button"
								onclick={() => imageInputEl?.click()}
								style="font-size:12px; color:#059669; background:none; border:none; cursor:pointer; padding:0; text-decoration:underline; text-underline-offset:2px; font-family:'Tajawal',sans-serif;"
							>
					...
					<div style="display:grid; grid-template-columns:1fr 1fr; gap:12px;">
						<div>
							<label class="field-label" for="m-name">الاسم</label>
							<input
								id="m-name"
								name="name"
								type="text"
					...
					<div class="section-card" style="padding:0; overflow:visible;">
						...
						>
							...
							<p style="font-size:11px; color:#8b909a; margin:0;">
								اختر من القائمة للإضافة إلى الطعام.
							</p>
						</div>
						<div style="padding:14px; display:flex; flex-direction:column; gap:14px;">
							<!-- Cautions multi-select -->
							<div>
								<p style="font-size:11.5px; font-weight:600; color:#92400e; margin:0 0 6px;">
									تحذيرات (مسببات حساسية)
								</p>
								<div class="multi-select">
									<button
										type="button"
										class="multi-trigger {cautionOpen ? 'open' : ''}"
										onclick={() => {
									...
									>
										<span
											>{fCautions.length === 0
												? 'اختر مسببات الحساسية...'
												: `${fCautions.length} محدد`}</span
										>
										<svg
											width="14"
											height="14"
											...
											stroke="currentColor"
											viewBox="0 0 24 24"
											style="transform:{cautionOpen
												? 'rotate(180deg)'
												: 'none'}; transition:.2s; flex-shrink:0; color:#8b909a;"
											><path
												stroke-linecap="round"
												stroke-linejoin="round"
												stroke-width="2"
									...
									{#if cautionOpen}
										<div class="multi-dropdown">
											{#each CAUTION_OPTIONS as opt}
												<label
													class="multi-option {fCautions.includes(opt.value) ? 'checked' : ''}"
												>
													<input
														type="checkbox"
														checked={fCautions.includes(opt.value)}
														onchange={() => (fCautions = toggleTag(fCautions, opt.value))}
													/>
													{opt.label}
												</label>
											{/each}
									...
									<div class="sel-tags">
										{#each CAUTION_OPTIONS.filter((o) => fCautions.includes(o.value)) as opt}
											<span class="sel-tag">
												...
												<button
													type="button"
													onclick={() => (fCautions = toggleTag(fCautions, opt.value))}
													aria-label="إزالة"
												>
													<svg
														width="10"
							...
							<!-- Diet multi-select -->
							<div>
								<p style="font-size:11.5px; font-weight:600; color:#065f46; margin:0 0 6px;">
									التصنيف الغذائي
								</p>
								<div class="multi-select">
									<button
										type="button"
										class="multi-trigger {dietOpen ? 'open' : ''}"
										onclick={() => {
											dietOpen = !dietOpen;
											cautionOpen = false;
											healthOpen = false;
									...
									>
										<span
											>{fDietLabels.length === 0
												? 'اختر التصنيف الغذائي...'
												: `${fDietLabels.length} محدد`}</span
										>
										<svg
											width="14"
											height="14"
									...
									{#if dietOpen}
										<div class="multi-dropdown">
											{#each DIET_OPTIONS as opt}
												<label
													class="multi-option {fDietLabels.includes(opt.value) ? 'checked' : ''}"
												>
													<input
														type="checkbox"
														checked={fDietLabels.includes(opt.value)}
														onchange={() => (fDietLabels = toggleTag(fDietLabels, opt.value))}
													/>
													{opt.label}
												</label>
											{/each}
									...
									<div class="sel-tags">
										{#each DIET_OPTIONS.filter((o) => fDietLabels.includes(o.value)) as opt}
											<span class="sel-tag">
												...
												<button
													type="button"
													onclick={() => (fDietLabels = toggleTag(fDietLabels, opt.value))}
													aria-label="إزالة"
												>
													<svg
														width="10"
							...
							<!-- Health multi-select -->
							<div>
								<p style="font-size:11.5px; font-weight:600; color:#0c4a6e; margin:0 0 6px;">
									خالٍ من / مناسب لـ
								</p>
								<div class="multi-select">
									<button
										type="button"
										class="multi-trigger {healthOpen ? 'open' : ''}"
										onclick={() => {
									...
									>
										<span
											>{fHealthLabels.length === 0
												? 'اختر ما ينطبق...'
												: `${fHealthLabels.length} محدد`}</span
										>
										<svg
											width="14"
											height="14"
									...
									{#if healthOpen}
										<div class="multi-dropdown">
											{#each HEALTH_OPTIONS as opt}
												<label
													class="multi-option {fHealthLabels.includes(opt.value) ? 'checked' : ''}"
												>
													<input
														type="checkbox"
														checked={fHealthLabels.includes(opt.value)}
														onchange={() => (fHealthLabels = toggleTag(fHealthLabels, opt.value))}
													/>
													{opt.label}
												</label>
											{/each}
									...
									<div class="sel-tags">
										{#each HEALTH_OPTIONS.filter((o) => fHealthLabels.includes(o.value)) as opt}
											<span class="sel-tag">
												...
												<button
													type="button"
													onclick={() => (fHealthLabels = toggleTag(fHealthLabels, opt.value))}
													aria-label="إزالة"
												>
													<svg
														width="10"
					...
					<div style="border:1px solid #e8eaed; border-radius:8px; overflow:hidden;">
						<div
							style="background:#edf9f2; padding:9px 14px; text-align:center; border-bottom:2px solid #3cb96b;"
						>
							<span style="font-size:13px; font-weight:600; color:#059669;"
								>القيمة الغذائية لكل 100 {fUnit === 'ml' ? 'مل' : 'غ'}</span
							>
						</div>
					</div>
					...
					<!-- Macros -->
					<div>
						<h3 style="font-size:13px; font-weight:600; color:#374151; margin:0 0 10px;">
							المغذيات الكبرى
						</h3>
						<div style="display:grid; grid-template-columns:repeat(4,1fr); gap:10px;">
							<div>
								<label
									for="m-create-calories"
									style="display:block; font-size:11.5px; font-weight:500; color:#6b7280; margin-bottom:4px;"
									>الطاقة</label
								>
								<div style="position:relative;">
									<input
										id="m-create-calories"
										type="number"
										name="calories"
										step="any"
										placeholder="0"
					...
					<div>
						...
						<div style="display:grid; grid-template-columns:repeat(3,1fr); gap:10px;">
							{#each MICRO_FIELDS as field}
								<div>
									<label
										for="m-create-micro-{field.key}"
										style="display:block; font-size:11.5px; font-weight:500; color:#6b7280; margin-bottom:4px;"
										>{field.label}</label
									>
									<div style="position:relative;">
										<input
											id="m-create-micro-{field.key}"
											type="number"
											name="micro_{field.key}"
											step="any"
											placeholder="0"
											class="nut-input"
				...
				<div class="modal-footer">
					<button
						type="button"
						onclick={() => (showCreateModal = false)}
						style="padding:9px 18px; border:1px solid #e8eaed; border-radius:8px; background:#fff; font-size:13.5px; color:#4b5563; cursor:pointer; font-family:'Tajawal',sans-serif;"
						>إلغاء</button
					>
					<button
						type="submit"
						class="btn-primary"
						disabled={!fName.trim()}
						style="opacity:{fName.trim() ? 1 : 0.5}; cursor:{fName.trim()
							? 'pointer'
							: 'not-allowed'};">حفظ وإغلاق</button
					>
				</div>
			</form>
		</div>
		...
		<div class="modal">
			<div class="modal-header">
				<div style="display:flex; align-items:flex-start; justify-content:space-between;">
					<div>
						...
						<p style="font-size:12px; color:#8b909a; margin:0;">
							حدّث البيانات بسرعة ثم احفظ التعديلات
						</p>
					</div>
					<button
						type="button"
			...
			>
				...
				<div class="modal-body">
					...
					<div style="display:grid; grid-template-columns:1fr 1fr; gap:12px;">
						<div>
							<label class="field-label" for="e-name-en">الاسم (English)</label>
							<input
								id="e-name-en"
								name="name"
								type="text"
					...
					<div class="section-card" style="padding:0; overflow:visible;">
						...
						>
							...
							<p style="font-size:11px; color:#8b909a; margin:0;">
								اختر من القائمة لتعديل تصنيفات الطعام.
							</p>
						</div>
						<div style="padding:14px; display:flex; flex-direction:column; gap:14px;">
							<div>
								...
								<div class="multi-select">
									<button
										type="button"
										class="multi-trigger {editCautionOpen ? 'open' : ''}"
										onclick={() => {
											editCautionOpen = !editCautionOpen;
											editDietOpen = false;
											editHealthOpen = false;
										}}
									>
										...
										<svg
											...
											stroke="currentColor"
											viewBox="0 0 24 24"
											style="transform:{editCautionOpen
												? 'rotate(180deg)'
												: 'none'}; transition:.2s; flex-shrink:0; color:#8b909a;"
											><path
												stroke-linecap="round"
									...
									{#if editCautionOpen}
										<div class="multi-dropdown">
											{#each CAUTION_OPTIONS as opt}
												<label
													class="multi-option {eCautions.includes(opt.value) ? 'checked' : ''}"
												>
													<input
														type="checkbox"
														checked={eCautions.includes(opt.value)}
														onchange={() => (eCautions = toggleTag(eCautions, opt.value))}
													/>
													{opt.label}
												</label>
											{/each}
							...
							<div>
								...
								<div class="multi-select">
									<button
										type="button"
										class="multi-trigger {editDietOpen ? 'open' : ''}"
										onclick={() => {
											editDietOpen = !editDietOpen;
											editCautionOpen = false;
											editHealthOpen = false;
										}}
									>
										<span
											>{eDietLabels.length === 0
												? 'اختر التصنيف الغذائي...'
												: `${eDietLabels.length} محدد`}</span
										>
										<svg
											width="14"
											height="14"
									...
									{#if editDietOpen}
										<div class="multi-dropdown">
											{#each DIET_OPTIONS as opt}
												<label
													class="multi-option {eDietLabels.includes(opt.value) ? 'checked' : ''}"
												>
													<input
														type="checkbox"
														checked={eDietLabels.includes(opt.value)}
														onchange={() => (eDietLabels = toggleTag(eDietLabels, opt.value))}
													/>
													{opt.label}
												</label>
											{/each}
							...
							<div>
								...
								<div class="multi-select">
									<button
										type="button"
										class="multi-trigger {editHealthOpen ? 'open' : ''}"
										onclick={() => {
											editHealthOpen = !editHealthOpen;
											editCautionOpen = false;
											editDietOpen = false;
										}}
									>
										<span
											>{eHealthLabels.length === 0
												? 'اختر ما ينطبق...'
												: `${eHealthLabels.length} محدد`}</span
										>
										<svg
											width="14"
											height="14"
											...
											stroke="currentColor"
											viewBox="0 0 24 24"
											style="transform:{editHealthOpen
												? 'rotate(180deg)'
												: 'none'}; transition:.2s; flex-shrink:0; color:#8b909a;"
											><path
												stroke-linecap="round"
									...
									{#if editHealthOpen}
										<div class="multi-dropdown">
											{#each HEALTH_OPTIONS as opt}
												<label
													class="multi-option {eHealthLabels.includes(opt.value) ? 'checked' : ''}"
												>
													<input
														type="checkbox"
														checked={eHealthLabels.includes(opt.value)}
														onchange={() => (eHealthLabels = toggleTag(eHealthLabels, opt.value))}
													/>
													{opt.label}
												</label>
											{/each}
					...
					<div style="border:1px solid #e8eaed; border-radius:8px; overflow:hidden;">
						...
						>
							<span style="font-size:13px; font-weight:600; color:#059669;"
								>القيمة الغذائية لكل 100 {eUnit === 'ml' ? 'مل' : 'غ'}</span
							>
						</div>
					</div>
						...
						<details class="micro-accordion">
							...
							<div class="micro-grid">
								{#each MICRO_FIELDS as field}
									<div>
										<label
											for="e-edit-micro-{field.key}"
											style="display:block; font-size:11.5px; font-weight:500; color:#6b7280; margin-bottom:4px;"
											>{field.label}</label
										>
										<div style="position:relative;">
											<input
												id="e-edit-micro-{field.key}"
												type="number"
												name="micro_{field.key}"
												bind:value={eMicros[field.key]}
												step="any"
												placeholder="0"
												class="nut-input"
												style="padding-left:32px;"
				...
				<div class="modal-footer">
					<div class="modal-footer-actions">
						<button
							...
							class="btn-primary"
							disabled={!eName.trim()}
							style="opacity:{eName.trim() ? 1 : 0.5}; cursor:{eName.trim()
								? 'pointer'
								: 'not-allowed'};">حفظ التعديلات</button
						>
...
<style>
	...
	.nut-input:focus {
		border-color: #3cb96b;
		box-shadow: 0 0 0 2px rgba(60, 185, 107, 0.15);
		background: #fff;
	}
	.nut-unit {
		position: absolute;
	...
	.spinner {
		width: 14px;
		height: 14px;
		border: 2px solid #b6e9ce;
		border-top-color: #3cb96b;
		border-radius: 50%;
