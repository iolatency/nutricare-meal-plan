...
const duplicateEmails = db
	.prepare<[], { email: string; cnt: number }>(
		`SELECT email, COUNT(*) as cnt FROM users GROUP BY email HAVING cnt > 1`
	)
	.all();
check('Email uniqueness', duplicateEmails.length === 0, `${duplicateEmails.length} duplicate emails found`);

const nullEmails = db
	.prepare<[], { cnt: number }>(`SELECT COUNT(*) as cnt FROM users WHERE email IS NULL OR email = ''`)
	.get()!;
...
const plainPasswords = db
	...
	.get()!;
check('Passwords are hashed (bcrypt)', plainPasswords.cnt === 0, `${plainPasswords.cnt} unhashed passwords`);

const duplicateUsernames = db
	.prepare<[], { username: string; cnt: number }>(
		`SELECT username, COUNT(*) as cnt FROM users WHERE username IS NOT NULL AND username != '' GROUP BY username HAVING cnt > 1`
	)
	.all();
check('Username uniqueness', duplicateUsernames.length === 0, `${duplicateUsernames.length} duplicate usernames`);
...
const orphanedSessions = db
	...
	.get()!;
check('No orphaned sessions', orphanedSessions.cnt === 0, `${orphanedSessions.cnt} orphaned sessions`);
...
const sessionsWithoutExpiry = db
	...
	.get()!;
check('All sessions have expiry', sessionsWithoutExpiry.cnt === 0, `${sessionsWithoutExpiry.cnt} sessions without expiry`);
...
if (mealPlanTableExists.cnt > 0) {
	const invalidStatus = db
		...
		.get()!;
	check('Meal plan session status values valid', invalidStatus.cnt === 0, `${invalidStatus.cnt} invalid statuses`);
	...
	const orphanedSessions2 = db
		...
		.get()!;
	check('All meal plan sessions have valid dietitian', orphanedSessions2.cnt === 0, `${orphanedSessions2.cnt} orphaned sessions`);
} else {
...
if (chatConvsTable.cnt > 0) {
	const orphanedConvs = db
		...
		.get()!;
	check('No orphaned chat conversations', orphanedConvs.cnt === 0, `${orphanedConvs.cnt} orphaned conversations`);
} else {
...
if (otpTable.cnt > 0) {
	const otpWithoutExpiry = db
		...
		.get()!;
	check('All OTP records have expiry', otpWithoutExpiry.cnt === 0, `${otpWithoutExpiry.cnt} OTPs without expiry`);
} else {
