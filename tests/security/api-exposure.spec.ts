...
test.describe('API Exposure — IDOR Protection', () => {
	...
	test('accessing conversation with very large numeric ID returns 4xx (not 5xx)', async ({
		page
	}) => {
	...
	test('POSTing to non-existent conversation returns 4xx (not 5xx)', async ({ page }) => {
		const res = await page.request.post('/api/chat/conversations/999999999/messages', {
			data: { content: 'hello' }
		});
		expect(res.status()).toBeGreaterThanOrEqual(400);
		expect(res.status()).toBeLessThan(500);
	...
	test('marking non-existent conversation as read returns 4xx (not 5xx)', async ({ page }) => {
		const res = await page.request.post('/api/chat/conversations/999999999/mark-read', {
			data: {}
		});
		expect(res.status()).toBeGreaterThanOrEqual(400);
		expect(res.status()).toBeLessThan(500);
	...
	test('PATCH supplement with non-existent ID returns 400 or no-op (global catalog)', async ({
		page
	}) => {
...
test.describe('API Exposure — Route-Level Permission Checks', () => {
	...
	test('SSE events endpoint requires valid conversation ownership', async ({ page }) => {
		const res = await page.request.get('/api/chat/conversations/999999/events');
